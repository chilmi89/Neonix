            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
                    ...
                    <motion.div
                        ...
                        className="relative w-full max-w-md max-h-[85vh] bg-[#121212] rounded-3xl overflow-y-auto shadow-2xl border border-white/10 flex flex-col"
                    >
                        {/* Header Image */}
                        <div className="relative h-40 shrink-0">
                            ...
                            <button
                                onClick={onClose}
                                className="absolute top-4 right-4 w-10 h-10 rounded-full bg-black/60 backdrop-blur-md flex items-center justify-center text-white hover:bg-neon-pink transition-colors border border-white/10 z-20"
                            >
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <h2 className="text-2xl font-black uppercase tracking-tighter">{ticket.title}</h2>
                                    <p className="text-xs font-bold text-neon-cyan mt-1 uppercase tracking-wider">{ticket.type}</p>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button className="flex items-center justify-center gap-2 py-3 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-neon-pink/10 hover:border-neon-pink/30 hover:text-neon-pink transition-all">
                                    <Download size={14} />
                                    Download QR
                                </button>
                                <button
                                    onClick={() => setIsTransferring(true)}
                                    className="flex items-center justify-center gap-2 py-3 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-neon-cyan/10 hover:border-neon-cyan/30 hover:text-neon-cyan transition-all"
                                >
                                    <Share2 size={14} />
                                    Transfer QR
                                </button>
